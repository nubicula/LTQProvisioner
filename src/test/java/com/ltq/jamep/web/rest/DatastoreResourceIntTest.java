...
public class DatastoreResourceIntTest {
    ...
    public void getAllDatastoresWithEagerRelationshipsIsEnabled() throws Exception {
        DatastoreResource datastoreResource = new DatastoreResource(datastoreRepositoryMock);
        when(datastoreRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restDatastoreMockMvc = MockMvcBuilders.standaloneSetup(datastoreResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restDatastoreMockMvc.perform(get("/api/datastores?eagerload=true"))
        .andExpect(status().isOk());

        verify(datastoreRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    public void getAllDatastoresWithEagerRelationshipsIsNotEnabled() throws Exception {
        DatastoreResource datastoreResource = new DatastoreResource(datastoreRepositoryMock);
            when(datastoreRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restDatastoreMockMvc = MockMvcBuilders.standaloneSetup(datastoreResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
