import { Component, OnInit, OnDestroy } from '@angular/core';
import { HttpErrorResponse, HttpResponse } from '@angular/common/http';
import { Subscription } from 'rxjs';
import { JhiEventManager, JhiAlertService } from 'ng-jhipster';

import { IDatastore } from 'app/shared/model/datastore.model';
import { Principal } from 'app/core';
import { DatastoreService } from './datastore.service';

@Component({
    selector: 'jhi-datastore',
    templateUrl: './datastore.component.html'
})
export class DatastoreComponent implements OnInit, OnDestroy {
    datastores: IDatastore[];
    currentAccount: any;
    eventSubscriber: Subscription;

    constructor(
        private datastoreService: DatastoreService,
        private jhiAlertService: JhiAlertService,
        private eventManager: JhiEventManager,
        private principal: Principal
    ) {}

    loadAll() {
        this.datastoreService.query().subscribe(
            (res: HttpResponse<IDatastore[]>) => {
                this.datastores = res.body;
            },
            (res: HttpErrorResponse) => this.onError(res.message)
        );
    }

    ngOnInit() {
        this.loadAll();
        this.principal.identity().then(account => {
            this.currentAccount = account;
        });
        this.registerChangeInDatastores();
    }

    ngOnDestroy() {
        this.eventManager.destroy(this.eventSubscriber);
    }

    trackId(index: number, item: IDatastore) {
        return item.id;
    }

    registerChangeInDatastores() {
        this.eventSubscriber = this.eventManager.subscribe('datastoreListModification', response => this.loadAll());
    }

    private onError(errorMessage: string) {
        this.jhiAlertService.error(errorMessage, null, null);
    }
}
